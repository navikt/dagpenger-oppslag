...
public class EndpointSTSClientConfig {
  ...
  private void configureEndpointWithPolicyForSTS(
      STSClient stsClient, Client client, String policyReference, boolean cacheTokenInEndpoint) {

    client
        .getRequestContext()
        .put(SecurityConstants.STS_CLIENT, stsClient);
  ...
  private void setClientEndpointPolicy(Client client, Policy policy) {
    ...
    EndpointPolicy endpointPolicy =
        policyEngine.getClientEndpointPolicy(endpointInfo, null, message);
    policyEngine.setClientEndpointPolicy(
        endpointInfo, endpointPolicy.updatePolicy(policy, message));
  }
}
